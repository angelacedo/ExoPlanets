...
const cors = Cors({
  ...
  oorigin: 'https://exoplanets.angelacedo.net/', // Permite todos los orígenes (esto debería cambiarse en producción para mayor seguridad)
});
export default async function handler(req: NextApiRequest, res: NextApiResponse)
{
  ...
  try
  {
    ...
    } else
    {
      ...
      const [rows]: [RowDataPacket[], FieldPacket[]] = await DBController.executeQuery(connection, sql, year == undefined ? [] : [year]);
      if (rows)
